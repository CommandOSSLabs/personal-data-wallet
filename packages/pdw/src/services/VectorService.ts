...
import { VectorConfig, VectorSearchOptions, VectorSearchResult, VectorSearchMatch } from '../types';

export class VectorService {
  private indices = new Map<string, HierarchicalNSW>();
  private vectorData = new Map<string, Map<number, { text: string; metadata: any }>>();
  ...
  loadIndex(userId: string, data: Uint8Array): boolean {
    try {
      ...
      // Recreate HNSW index
      const index = new HierarchicalNSW('cosine', combined.config.dimensions);
      ...
      // Restore user data
      const userDataArray: [number, { text: string; metadata: any }][] = JSON.parse(combined.userData);
      const userData = new Map<number, { text: string; metadata: any }>(userDataArray);
