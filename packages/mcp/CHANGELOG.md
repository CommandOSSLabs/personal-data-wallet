...
- Answer tool calls with an auth error when the relayer rejects the saved delegate key, instead of parking them until the call deadline. A 401 on the SSE handshake was treated like any other connect failure, so the background connect backed off and retried a key that could never be accepted while the queued `memwal_recall` waited out the orphan sweeper — up to four minutes — and then came back as "the connection to the relayer dropped, please retry", advice that cannot work. The bridge now names the rejection and points at `memwal_login`, and refuses later requests immediately while the key stays rejected; a successful connect (after `memwal_login`, or once a transient 401 clears) resumes normal buffering. Credentials are still never wiped automatically. (#365, WALM-602)
