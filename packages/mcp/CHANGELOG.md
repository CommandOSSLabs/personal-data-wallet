...
- Retry a timed-out read and answer it with a structured error instead of an opaque stall. A `memwal_recall` whose reply never arrived was closed out on the first deadline with prose only, so an agent could not tell an overloaded relayer from a dropped connection or a misconfigured bridge, and had no signal that retrying was worthwhile. Timed-out reads (`memwal_recall`, `memwal_health`) are now replayed on a fresh session up to `MEMWAL_MCP_CALL_RETRIES` times (default 2) using the existing reconnect backoff; when those run out the tool result carries a `structuredContent` payload with `code`, `class` (`relayer_overload` / `transient_network` / `bridge_misconfigured`), `attempts` (calls that actually went out, counted at the POST rather than when a retry is decided on, so a retry the bridge could not send is never reported as spent), `retryable`, and `nextStep`. Writes are never replayed, since the relayer may already have applied the first attempt. (WALM-393)
- Keep a timed-out read's retry from duplicating a concurrent write. The retry reconnects, and a reconnect replays every outstanding request, so a `memwal_recall` giving up could re-send a `memwal_remember` the relayer had already accepted on a healthy stream and store it twice. Writes in flight when a read times out are now excluded from that replay and left to expire into their own error. Replay after a genuine stream loss is unchanged, since there the POST itself never landed. (WALM-393)
- Never send a call the bridge has already answered. A request that arrives while the bridge is reconnecting parks on that reconnect before posting, and its per-call deadline can expire while it is parked — so a `memwal_remember` could be answered with a timeout error and then POSTed to the relayer anyway, storing a memory the agent had just been told was not stored. An agent following the error's own `nextStep` then wrote it a second time. The forwarding path now drops a request that has already been answered, or that the reconnect replayed while it waited, and the POST helper re-checks at the moment of sending. (WALM-393)
- Stop telling an agent to repeat a write that may already have landed. A call only times out after the relayer accepted its POST, which is why the bridge refuses to replay a write itself — but the answer it handed back still carried `retryable: true` and "retry the same call", moving the duplicate one level up to the agent. A sent write now reports `retryable: false` and a `nextStep` that asks for a `memwal_recall` check before re-sending. Reads, and any call that never left the bridge (`attempts: 0`), are unchanged. (WALM-393)
- Stop reporting a failed message POST as unparseable client input. One `try` wrapped both `JSON.parse` and the forwarding `await`s, so any transport error was logged as `bridge.stdin_parse_failed` and the request was dropped with no reply at all. Parse failures and send failures are now separate, and a send failure answers the caller with the same structured error. (WALM-393)
