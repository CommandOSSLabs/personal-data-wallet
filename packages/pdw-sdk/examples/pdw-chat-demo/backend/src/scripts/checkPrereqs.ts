...
async function checkPostgres(connectionString: string, useSsl: boolean): Promise<CheckResult> {
...
async function checkSuiRpc(url: string): Promise<CheckResult> {
  try {
...
async function checkGemini(apiKey: string, model: string): Promise<CheckResult> {
  try {
...
async function checkWalrus(relay?: string): Promise<CheckResult | null> {
  if (!relay) {
    return null;
  }
