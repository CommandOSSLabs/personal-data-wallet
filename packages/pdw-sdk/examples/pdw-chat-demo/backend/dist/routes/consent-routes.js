...
async function resolveConsentRequest(pdw, repository, requestId, targetWallet) {
    if (repository) {
    ...
    return pendingForUser.find((request) => request.requestId === requestId) ?? null;
}
export function createConsentRouter(pdw, repository) {
    const router = Router();
