...
export class WalletManagementService {
  ...
  private walletRegistryId: string;
  ...
  async createMainWallet(signer: Signer): Promise<{ digest: string; walletId: string }> {
    ...
    const mainWalletObject = createdObjects?.find((obj: any) =>
      obj.objectType?.includes('::wallet::MainWallet')
    ) as any;
  ...
  async createContextWallet(
    ...
    const contextWalletObject = createdObjects?.find((obj: any) =>
      obj.objectType?.includes('::wallet::ContextWallet')
    ) as any;
   ...
   * @returns Transaction object
   */
  buildCreateContextWalletTransaction(
    options: CreateContextWalletOptions,
