 ...
 */
export function useMemoryChat(userAddress, options = {}) {
    const { systemPrompt = 'You are a helpful assistant with access to user memories.', maxContextMemories = 5, aiProvider = 'gemini', streamResponses = false, config, sessionId = 'default', autoSaveMessages = false, geminiApiKey, } = options;
    const account = useCurrentAccount();
    ...
    const { search, results: searchResults } = useMemorySearch(userAddress, geminiApiKey ? { geminiApiKey } : undefined);
    // Create memory hook
    ...
    const generateAIResponse = useCallback(async (userMessage, context) => {
        ...
        const contextText = context
            .map((mem, i) => `Memory ${i + 1} (similarity: ${mem.similarity.toFixed(2)}):\n${mem.content}`)
            .join('\n\n');
...
${contextText ? `Relevant memories from the user:\n${contextText}\n\n` : ''}User message: ${userMessage}`;
     ...
     */
    const createMemoryFromMessage = useCallback(async (content) => {
        if (!content.trim())
            return;
        if (!account || !client) {
...
//# sourceMappingURL=useMemoryChat.js.map