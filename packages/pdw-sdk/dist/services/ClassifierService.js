...
class ClassifierService {
     ...
     */
    async shouldSaveMemory(message, options = {}) {
        try {
            // Input validation
            ...
            const { useAI = true, confidenceThreshold = 0.7, patterns = this.factPatterns } = options;
     ...
     */
    async analyzePatterns(text, patterns = this.factPatterns) {
        const matches = [];
        const foundPatterns = [];
     ...
     * @returns Classification result
     */
    async classifyWithAI(message, options) {
     ...
     * @returns Classification result
     */
    async classifyWithEmbeddings(message, options) {
        if (!this.embeddingService) {
