...
export class BrowserKnowledgeGraphManager {
     ...
     */
    async updateUserGraph(userId, graph) {
        try {
            // Update cache
            this.graphCache.set(userId, { graph, lastUpdated: new Date() });
            // Save to IndexedDB
     ...
     */
    async addToGraph(userId, entities, relationships, memoryId) {
        try {
            // Get or create graph
            ...
            for (const relationship of relationships) {
                const existing = graph.relationships.find(r => r.source === relationship.source && r.target === relationship.target && r.label === relationship.label);
                if (!existing) {
     ...
     */
    async findRelatedEntities(userId, entityId, maxHops = 2) {
        try {
            const graph = await this.getUserGraph(userId);
            if (!graph) {
                return [];
    ...
    async saveGraphToDB(userId, graph) {
        if (!this.db) {
            await this.initializeIndexedDB();
        }
        return new Promise((resolve, reject) => {
...
//# sourceMappingURL=BrowserKnowledgeGraphManager.js.map