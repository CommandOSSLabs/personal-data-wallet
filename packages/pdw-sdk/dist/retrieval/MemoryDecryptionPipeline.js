...
class MemoryDecryptionPipeline {
    constructor(encryptionService, storageManager, config) {
        ...
        this.sessionKeyTimestamps = new Map();
        // Decryption cache
     ...
     */
    async initializeDecryptionPipeline() {
        try {
     ...
     */
    async decryptMemoryBatch(requests) {
     ...
     */
    async performDecryption(request, sessionKey) {
        if (!this.sealClient) {
        ...
        const txBytes = await accessTx.build({ client: this.encryptionService.suiClient });
     ...
     */
    async verifyAccess(userAddress, memoryId, ownerAddress) {
        try {
     ...
     */
    async verifyContentIntegrity(content, expectedHash) {
        try {
            return await this.encryptionService.verifyContentHash(content, expectedHash);
        }
        catch (error) {
     ...
     */
    async retryFailedDecryptions(originalRequests, failed, successful) {
    ...
    async getSessionKeyId(sessionKey) {
        try {
    ...
    updateDecryptionStats(success, processingTime) {
    ...
    generateBatchStats(totalRequests, successCount, failureCount, totalProcessingTime) {
        return {
            totalRequests,
            successCount,
            failureCount,
     ...
     */
    async decryptMemoryResults(memories, userAddress) {
        ...
        const decryptionRequests = encryptedMemories.map(memory => ({
            memoryId: memory.id,
        ...
        if (this.config.enableBatchDecryption && decryptionRequests.length > 1) {
            ...
            if (batchResult.failed.length > 0) {
                console.warn(`⚠️ Failed to decrypt ${batchResult.failed.length} memories:`, batchResult.failed.map(f => f.memoryId));
            }
        }
        else {
...
MemoryDecryptionPipeline.VERIFIED_TESTNET_SERVERS = [
    {
...
//# sourceMappingURL=MemoryDecryptionPipeline.js.map