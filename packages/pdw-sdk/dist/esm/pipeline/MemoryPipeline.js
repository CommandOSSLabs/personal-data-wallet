...
export class MemoryPipeline {
     ...
     */
    async processMemory(memory, userId, options = {}) {
     ...
     */
    async processMemoriesBatch(memories, userId, options = {}) {
        ...
        if (options.enableParallel) {
            ...
            const promises = memories.map(memory => this.processMemory(memory, userId, {
    ...
    shouldExecuteStep(stepName, skipSteps) {
        return !skipSteps?.includes(stepName);
    }
    async executeEmbeddingStep(execution, memory) {
    ...
    async executeVectorIndexingStep(execution, memory) {
        ...
        return this.executeStep(step, async () => {
            ...
            const vectorResult = await this.vectorManager.addTextToIndex(memory.userId || 'default-user', memory.content, {
    ...
    async executeKnowledgeGraphStep(execution, memory, userId) {
    ...
    async executeStorageStep(execution, memory, userId, options) {
    ...
    async executeBlockchainStep(execution, memory, userId, options) {
        const step = this.findStep(execution, 'blockchain_record');
        return this.executeStep(step, async () => {
            console.log(`⛓️ Creating blockchain record for memory: ${memory.id}`);
            const ownershipRecord = await this.blockchainManager.createMemoryRecord(memory, userId, {
                enableBatching: this.config.blockchain.enableBatching,
                customMetadata: options.customMetadata
            });
            return {
    ...
    async executeStep(step, operation) {
