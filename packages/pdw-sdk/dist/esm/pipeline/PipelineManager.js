...
export class PipelineManager {
     ...
     */
    createPipeline(name, config = {}, options = {}) {
     ...
     */
    createPipelineFromTemplate(templateName, customName, configOverrides = {}) {
     ...
     */
    async processMemory(pipelineId, memory, userId, options) {
        const managedPipeline = this.pipelines.get(pipelineId);
        if (!managedPipeline) {
     ...
     */
    async processMemoriesBatch(pipelineId, memories, userId, options) {
        const managedPipeline = this.pipelines.get(pipelineId);
        if (!managedPipeline) {
            throw new Error(`Pipeline not found: ${pipelineId}`);
        }
     ...
     */
    scheduleMemoryProcessing(pipelineId, userId, memories, scheduledAt, priority = 'normal') {
    ...
    // ==================== PRIVATE METHODS ====================
    async executeScheduledProcessing(scheduleId) {
        const schedule = this.schedules.get(scheduleId);
        if (!schedule || schedule.status !== 'scheduled') {
            return;
        }
        ...
        try {
            ...
            const executions = await this.processMemoriesBatch(schedule.pipelineId, schedule.memories, schedule.userId);
    ...
    updateSystemMetrics() {
        ...
        this.systemMetrics.performance = {
            ...
            throughput: Array.from(this.pipelines.values()).reduce((sum, p) => sum + p.pipeline.getPipelineMetrics().throughput.memoriesPerHour, 0),
    ...
    updatePerformanceMetrics(managedPipeline) {
