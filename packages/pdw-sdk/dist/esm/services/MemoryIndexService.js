...
export class MemoryIndexService {
    constructor(storageService, options = {}) {
        ...
        this.hnswService = new HnswIndexService(storageService || undefined, {
     ...
     */
    initialize(embeddingService, storageService) {
     ...
     */
    async indexMemory(userAddress, memoryId, blobId, content, metadata, embedding) {
        try {
     ...
     */
    async getUserMemories(userAddress, filters) {
        ...
        for (const [memoryId, entry] of userMemories) {
            ...
            if (filters) {
                ...
                if (filters.importanceRange) {
                    ...
                    if (filters.importanceRange.min && importance < filters.importanceRange.min)
                        continue;
                    if (filters.importanceRange.max && importance > filters.importanceRange.max)
                        continue;
                }
                if (filters.dateRange) {
                    const created = new Date(entry.metadata.createdTimestamp || 0);
                    if (filters.dateRange.start && created < filters.dateRange.start)
                        continue;
                    if (filters.dateRange.end && created > filters.dateRange.end)
                        continue;
                }
            }
            results.push({
     ...
     */
    async removeMemory(userAddress, memoryId) {
        try {
            const userMemories = this.memoryIndex.get(userAddress);
            if (!userMemories) {
                return false;
     ...
     */
    async loadIndex(userAddress, indexBlobId) {
        if (indexBlobId) {
     ...
     */
    calculateAdvancedRelevanceScore(similarity, metadata, query, queryVector, documentVector) {
     ...
     */
    calculateRecencyBoost(createdTimestamp) {
        const now = Date.now();
        const ageInDays = (now - createdTimestamp) / (1000 * 60 * 60 * 24);
        // Exponential decay: more recent = higher boost
        if (ageInDays < 1)
            return 1.0; // Last day: full boost
        if (ageInDays < 7)
            return 0.8; // Last week: 80% boost
        if (ageInDays < 30)
            return 0.5; // Last month: 50% boost
        if (ageInDays < 90)
     ...
     */
    updateSearchStats(userAddress, latency) {
