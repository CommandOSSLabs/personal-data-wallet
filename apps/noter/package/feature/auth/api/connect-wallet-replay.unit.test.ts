...
describe("getChallenge", () => {
  it("issues a signin-purpose challenge with nonce alias for the requested address", async () => {
    const c = await caller();
    const result = await c.getChallenge({ address: ADDR });

    expect(result.challengeId).toBe(CHALLENGE_ID);
    expect(result.nonce).toBe("Sign in to Walrus Memory Noter");
    expect(result.message).toBe("Sign in to Walrus Memory Noter");
