...
async function testSealComplete() {
  ...
  try {
    ...
    // Test 1: Basic self encryption/decryption
    ...
    // Test 4: Multiple encryptions with same identity
    ...
    for (let i = 0; i < items.length; i++) {
      const encrypted = await sealIBEService.encryptForApp(
        items[i],
        userAddress,
        appAddress
      );
    ...
    for (let i = 0; i < encryptedItems.length; i++) {
      try {
        const decrypted = await sealIBEService.decryptWithIdentity(
    ...
    // Test 5: Session persistence across service instances
