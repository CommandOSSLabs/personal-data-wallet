...
app.post('/request-key', async (req, res) => {
    try {
        ...
        const sessionKey = await sealClient.createSessionKey(userAddress, packageId, keyRequest.signature);
...
app.post('/decrypt', async (req, res) => {
    try {
        ...
        const recreatedSessionKey = await sealClient.createSessionKey(sessionKeyData.userAddress, sessionKeyData.packageId);
        ...
        const decryptedData = await sealClient.decrypt(encryptedObject, recreatedSessionKey, txBytes);
        const response = {
