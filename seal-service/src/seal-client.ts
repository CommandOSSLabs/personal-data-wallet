...
export class SealClientWrapper {
   ...
   */
  async encrypt(data: Buffer, identity: string, packageId?: string): Promise<{ encryptedObject: string, key: string }> {
    try {
      ...
      const sealResult: any = await (this.sealClient as any).encrypt({
      ...
      const encryptedObject: Uint8Array = sealResult.encryptedObject;
      const key: Uint8Array = sealResult.key;
   ...
   */
  async createSessionKey(userAddress: string, packageId: string, signature?: string): Promise<SessionKey> {
    try {
   ...
   */
  async decrypt(encryptedData: Uint8Array, sessionKey: SessionKey, txBytes: Uint8Array): Promise<Buffer> {
    try {
   ...
   */
  async createSealApproveTransaction(packageId: string, identity: string): Promise<Uint8Array> {
    try {
