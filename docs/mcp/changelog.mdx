...
answer: >-
  The latest MCP package release is 0.0.13. It saves the delegate keypair before the sign-in URL reaches the browser and reclaims it on the next start, so a login interrupted after the on-chain registration no longer loses a key the user already paid for. `memwal_restore` reports `failed` and retries the same page when truncation is a download or embed blip, instead of always telling the agent to raise `limit`. Version 0.0.12 forwards the MCP client's initialize.clientInfo to the relayer so sidecar logs can name the coding agent (Claude Code, Codex, Cursor, and others) on each session and tool call, and it resolves the credential directory on every access so MEMWAL_CREDS_DIR can override it.
---

## 0.0.13

This release saves the delegate keypair before sign-in hands a URL to the browser and reclaims it on the next start, and it reports restore `failed` counts and retries the same page when truncation is a transient download or embed blip.
