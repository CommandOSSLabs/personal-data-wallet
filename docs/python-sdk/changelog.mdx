...
This release documents that `wait_for_remember_job` raises `MemWalRememberJobTimeout` on poll timeout while `wait_for_remember_jobs` returns `status="timeout"`.
