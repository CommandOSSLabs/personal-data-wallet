...
answer: >-
  The latest TypeScript SDK release is 0.1.7. `restore()` results include `failed` (required like `truncated`; SDK defaults omitted to `0`) for permanent decrypt/UTF-8 failures instead of folding them into `skipped`. Empty-body 401s use the AUTH_REJECTED troubleshooting message instead of telling callers to run `memwal_login`. Account and manual PTBs use typed `tx.pure` helpers so they work with modern `@mysten/sui`. Omitting `suiClient` on `createAccount` / `addDelegateKey` / `MemWalManual` constructs `SuiJsonRpcClient` on `@mysten/sui` 2.6+ instead of throwing `SuiClient not found`. 0.1.6 added optional `created_at` on `recall()` results, plus `sort` and `scoringWeights` on `RecallOptions`, and reports HTTP 503 as a retryable upstream outage instead of a sign-in failure.
---

## 0.1.7

This release adds `failed` on `restore()` results, stops telling headless SDK clients to call `memwal_login` on empty-body 401s, switches account and manual PTBs to typed `tx.pure` helpers, and constructs a JSON-RPC Sui client on `@mysten/sui` 2.6+ when `suiClient` is omitted.
