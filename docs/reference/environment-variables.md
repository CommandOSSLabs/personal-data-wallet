...
| `MEMWAL_AUTH_REVERIFY_INTERVAL_SECS` | `45` | How long a signed request may authenticate from a previous successful on-chain delegate-key verification before the relayer re-issues the Sui `GetObject`. Scoped to the exact `(delegate public key, account object id)` pair that verified. Allowed range `0`–`60`; `0` re-verifies on every request (the pre-window behaviour); out-of-range or unparseable values fall back to `45`. This is the revocation-visibility bound: a delegate key revoked on-chain keeps working for at most one window before the next request re-verifies and evicts it. Widening it absorbs more fullnode `429` pressure; narrowing it tightens revocation latency |
