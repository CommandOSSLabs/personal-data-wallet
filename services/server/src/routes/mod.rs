...
mod writes_paused_route_tests {
    ...
    use crate::types::{
        reject_if_writes_paused, writes_health_status, AppError, WRITES_PAUSED_ERROR,
    };
    ...
    const READ_PATHS: &[&str] = &["/api/recall", "/api/restore"];

    async fn write(State(paused): State<bool>) -> Result<StatusCode, AppError> {
        reject_if_writes_paused(paused)?;
    ...
    async fn health(State(paused): State<bool>) -> Json<serde_json::Value> {
        Json(serde_json::json!({
            "status": "ok",
            "writes": writes_health_status(paused),
        }))
    }

    fn app(paused: bool) -> Router {
        Router::new()
            .route("/api/remember", post(write))
            .route("/api/remember/manual", post(write))
            .route("/api/remember/bulk", post(write))
            .route("/api/analyze", post(write))
            .route("/api/recall", post(read))
            .route("/api/restore", post(read))
            .route("/health", get(health))
            .with_state(paused)
    }

    async fn send(app: &Router, method: &str, path: &str) -> axum::http::Response<Body> {
        app.clone()
            .oneshot(
                Request::builder()
                    .method(method)
    ...
    #[tokio::test]
    async fn paused_writes_return_503_while_reads_and_health_stay_ok() {
        let paused = app(true);

        for path in WRITE_PATHS {
            let resp = send(&paused, "POST", path).await;
            assert_eq!(resp.status(), StatusCode::SERVICE_UNAVAILABLE, "{path}");
            let json = json_body(resp).await;
            assert_eq!(json["error"], WRITES_PAUSED_ERROR, "{path}");
        }
        ...
        let health_resp = send(&paused, "GET", "/health").await;
        assert_eq!(health_resp.status(), StatusCode::OK);
        let json = json_body(health_resp).await;
        assert_eq!(json["status"], "ok");
        assert_eq!(json["writes"], "paused");
    }

    #[tokio::test]
    async fn writes_are_accepted_when_not_paused() {
        let open = app(false);

        for path in WRITE_PATHS {
            let resp = send(&open, "POST", path).await;
            assert_eq!(resp.status(), StatusCode::ACCEPTED, "{path}");
        }

        let health_resp = send(&open, "GET", "/health").await;
        assert_eq!(health_resp.status(), StatusCode::OK);
        let json = json_body(health_resp).await;
        assert_eq!(json["status"], "ok");
